...
const ViewApplications = () => {
  ...
  const fetchCompanyJobApplications = async () => {
    try {
      ...
      if (data.success && Array.isArray(data.applicants)) { // <-- Check if data.applicants exists and is an array
            ...
            {applicants.filter( item => item.jobId && item.userId).map((applicant, index) => (
              <tr key={index} className='text-gray-700'>
                ...
                <td className='py-2 px-4 border-b text-center flex items-center'>
                  <img className='w-10 h-10 rounded-full mr-3 max-sm:hidden' src={applicant.userId.imagem} alt="" />
                ...
                <td className='py-2 px-4 border-b relative'>
                  ...
                  ? <div className='relative inline-block text-lef group'>
                  <button className='text-gray-500 action-button'>...</button>
                  <div className='z-10 hidden absolute right-0 md:left-0 top-0 mt-2 w-32 bg-white boder border-gray-200 rounded shadow group-hover:block'>
                    ...
                    <button onClick={() => changeJobApplicationStatus(applicant._id, 'Rejected')} className='block w-full text-left px-4 py-2 text-gray-500 hover:bg-gray-100'>Reject</button>
                  </div>
                </div>
